...
| [0008](docs/adr/0008-deferred-to-plan-3.md) | HPA is permanently out of scope (kind ships no metrics-server); the ServiceMonitor deferral is closed as the operator's seventh owned child; `PlantSpec.chaos` stays dropped rather than shipped inert. |
...
Two things a careful reviewer will notice and might otherwise read as bugs:
