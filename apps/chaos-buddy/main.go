...
func init() {
	cfg := &chaosConfig{
		...
		killInterval:  getEnvInt("KILL_INTERVAL_SECONDS", 60),
	}
...
func flipConfigMap() error {
	...
	configData := "primary"
	if os.Getenv("CONFIG_MODE") == "chaos" {
		configData = "chaos"
	}
	...
	if cm.Data["mode"] == "primary" {
		cm.Data["mode"] = "chaos"
	} else {
		cm.Data["mode"] = "primary"
	}
	...
	log.Printf("🔄 ConfigMap %s flipped! Shaking things up!", configMapName)
	return nil
}

func chaosLoop(killInterval int) {
	ticker := time.NewTicker(time.Duration(killInterval) * time.Second)
	defer ticker.Stop()

	log.Printf("🚀 Chaos loop started! Interval: %d seconds", killInterval)
...
func statusHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	fmt.Fprintf(w, `{"chaos_enabled": %v, "namespace": "%s"}`, enabled, namespace)
}
...
func homeHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(http.StatusOK)
	fmt.Fprintf(w, `{"status": "alive", "message": "👋 Welcome to Chaos Buddy!", "chaos_enabled": %v}`, enabled)
}
...
func main() {
	...
	mu.Lock()
	chaosMode = getEnvBool("CHAOS_ENABLED", false)
	mu.Unlock()

	go chaosLoop(killInterval)
